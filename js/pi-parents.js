...
function calculatePi(rowID){
    if(rowID > 0){
        ...
        alert("/expriment/" + locus.value + ".xml");
...
function getAllete(exprimentfile,allete){
    if (window.exprimentHttpRequest){
        exprimenthttp = new exprimentHttpRequest();
    }else{
        exprimenthttp = new ActiveXObject("Microsoft.exprimentHTTP");
    }
    exprimenthttp.open("GET",exprimentfile,false);
    exprimenthttp.send();
    exprimentDoc = exprimenthttp.responseexpriment;
    return(exprimentDoc.getElementsByTagName(allete)[0].childNodes[0].nodeValue);
}